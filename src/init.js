...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('microscope_state', () => {
        return {
            ...
            get num_images() {
                const num_sites_xy = this.microscope_config.grid.masks.reduce((o, n) => o + (n.selected ? 1 : 0), 0)
            ...
            Actions: {
                 ...
                 */
                moveBy(axis, distance_mm) {
                    return fetch("http://localhost:5002/api/action/move_by", {
                        method: "POST",
                        body: JSON.stringify({ axis, distance_mm }),
                        headers: [
            ...
            callback_setStreamingChannel(element) {
                console.log(`setting live channel to ${element.value}`)
            ...
            */
            async updatePlate(newplate_Model_id, force_override) {
                ...
                this.microscope_config.grid.masks.splice(0, this.microscope_config.grid.masks.length, ...new_masks)
